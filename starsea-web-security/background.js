/**
 * 星海 Web 安全 - Background Service Worker (MV3) v1.2.0
 *
 * v1.2.0:
...
async function init() {
  ...
  setupListeners();
  setupAlarms();
  console.log('[星海安全] v1.2.0 initialized', {
...
async function loadBlocklist() {
  try {
    ...
    console.error('[星海安全] Failed to load blocklist:', e);
    try {
      const bundled = await fetch(chrome.runtime.getURL('rules/blocklist.json'));
      const data = await bundled.json();
      matcher = new IOCMatcher(data, {});
    } catch (e2) {
      console.error('[星海安全] Fatal: could not load any blocklist', e2);
    }
  }
}
...
function showBlockNotification(result) {
  ...
  chrome.notifications.create({
    ...
    title: `${icons[result.severity] || '⚠️'} 星海安全已拦截`,
    message: `威胁: ${result.family || '未知威胁'}\n${result.context?.substring(0, 100) || ''}`,
    priority: 2
  });
}
...
async function checkDownload(downloadItem) {
  ...
  if (result && !result.allowed) {
    ...
    await recordBlock({ ...result, context: '恶意下载: ' + (result.context || '') });

    chrome.notifications.create({
      type: 'basic',
      iconUrl: 'icons/icon128.png',
      title: '🚫 恶意下载已拦截',
      message: `文件来自已知恶意源: ${result.family || '未知'}\nURL: ${url.substring(0, 80)}`,
      priority: 2
    });
  }
}
  ...
  try {
    ...
    if (resp.ok) {
      ...
      if (data && data.domains) {
        ...
        console.log('[星海安全] Blocklist updated remotely', data.version);
      }
    }
  } catch (e) {
    console.log('[星海安全] Remote update failed:', e.message);
  }
}
