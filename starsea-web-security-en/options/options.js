/**
 * StarSea Web Security - Options Page Script
 */
...
async function init() {
  ...
  $('saveUrl').addEventListener('click', () => {
    ...
    chrome.runtime.sendMessage({ action: 'updateSettings', settings: { blocklistUrl: url } }, () => {
      alert('Saved. Will sync on next update cycle.');
    });
  });

  $('reloadRules').addEventListener('click', () => {
    $('reloadRules').textContent = 'Loading...';
    chrome.runtime.sendMessage({ action: 'reloadBlocklist' }, () => {
      $('reloadRules').textContent = 'Reload Built-in Rules';
      init();
    });
  });
  ...
  $('clearStats').addEventListener('click', () => {
    if (confirm('Are you sure you want to clear all block logs?')) {
  ...
  $('saveCloud').addEventListener('click', () => {
    ...
    chrome.storage.local.set({ starsea_cloud: cloudConfig }, () => {
      chrome.runtime.sendMessage({ action: 'cloudUpdate' }, () => {
        $('cloudStatus').textContent = '✓ Cloud config saved';
  ...
  $('testCloud').addEventListener('click', async () => {
    ...
    if (!endpoint || !apiKey) {
      $('cloudStatus').textContent = 'Please enter API endpoint and key first';
      $('cloudStatus').style.color = '#f87171';
      return;
    }
    $('cloudStatus').textContent = 'Testing...';
    $('cloudStatus').style.color = '#94a3b8';
    try {
      const resp = await fetch(`${endpoint}/health`);
      if (resp.ok) {
        const data = await resp.json();
        $('cloudStatus').textContent = `✓ Connected - ${data.service} v${data.version}`;
        $('cloudStatus').style.color = '#4ade80';
      } else {
      ...
      $('cloudStatus').textContent = `✗ Connection failed: ${e.message}`;
      $('cloudStatus').style.color = '#f87171';
    }
  });
}
...
function loadCustomRules() {
  chrome.runtime.sendMessage({ action: 'getCustomRules' }, (rules) => {
    ...
    if (all.length === 0) {
      container.innerHTML = '<div style="color:#475569; font-size:12px; text-align:center; padding:12px;">No custom rules yet</div>';
      return;
    }
    container.innerHTML = all.map(r => `
      <div class="rule-item">
        <span><strong style="color:#60a5fa">[${r.type}]</strong> ${r.value} <span style="color:#64748b">${r.context || ''}</span></span>
        <span class="del" data-value="${r.value}">✕ Remove</span>
      </div>
    `).join('');
...
function renderBlockLog(blocks) {
  ...
  if (!blocks || blocks.length === 0) {
    container.innerHTML = '<div style="color:#475569; font-size:13px; text-align:center; padding:20px;">🛡️ No threats blocked yet — you are protected</div>';
    return;
  }
  container.innerHTML = blocks.map(b => {
    const time = new Date(b.time).toLocaleString('en-US');
    ...
    return `
      <div style="padding:10px; background:#0f172a; border-radius:6px; margin-bottom:6px; border-left:3px solid ${color};">
        <div style="display:flex; justify-content:space-between;">
          <strong style="color:${color}">${b.family || 'Unknown Threat'}</strong>
...
// v1.2.0: Allowlist loading and rendering
function loadAllowlist() {
  chrome.runtime.sendMessage({ action: 'getAllowlist' }, (data) => {
    ...
    if (entries.length === 0) {
      container.innerHTML = '<div style="color:#475569; font-size:12px; text-align:center; padding:12px;">No custom allowlist entries</div>';
      return;
    }
    ...
    container.innerHTML = entries.map(e => `
      <div class="rule-item" style="border-left:3px solid ${typeColors[e.type] || '#6ee7b7'};">
        ...
        <span class="del-allow" data-value="${e.value}" style="color:#f87171;cursor:pointer;">✕ Remove</span>
      </div>
    `).join('');
